...
struct page {
	...
	unsigned int flags;
	int order;
	struct kmem_cache *cachep; // page 和 slab 之间的联系
...
int find_right_order(unsigned int size){ // return 0(0-410)  1(411-820) - 5(2051-2460)
	int order;
	for(order=0;order<=KMEM_CACHE_MAX_ORDER;order++){ // 5
		if(size<=(KMEM_CACHE_MAX_WAST)*(1<<order)){ //0x19a  * 2^order // 410 *1 // 410 *2 // 410 * 2^32
			return order;
		}
	}
	if(size>(1<<order))
...
int kmem_cache_line_object(void *head,unsigned int size,int order){ // size 为 32 64 4096 // order 为 0 ~ 10
	void **pl;
	char *p;
	pl=(void **)head;
	p=(char *)head+size;
...
struct kmem_cache *kmem_cache_create(struct kmem_cache *cache,unsigned int size,unsigned int flags){
	...
	int order=find_right_order(size); // size 应该 小于 32 * 2^32
	if(order==-1)
		return NULL;
	if((cache->head_page=alloc_pages(0,order))==NULL)  // 返回 第一个page 的地址
		return NULL;
	*nf_block=page_address(cache->head_page); // 得到虚拟地址
...
void kmem_cache_free(struct kmem_cache *cache,void *objp){
	*(void **)objp=cache->nf_block; // 得到上一块内存地址
	cache->nf_block=objp;  // 指向上一块内存地址
	cache->obj_nr++;  // 可用内存块加1
}
...
void *kmem_cache_alloc(struct kmem_cache *cache,unsigned int flag){
	...
	(*nr)--; // 已经有内存块了, 剩余内存块 减 1
	p=*nf_block; // 得到 当前内存块的虚拟地址
	*nf_block=*(void **)p; // nf_block 指向下一块地址
...
int kmalloc_init(void){
    ...
    printk("find_right_order 32:%d\n",find_right_order(32));
    printk("find_right_order 34:%d\n",find_right_order(34));
    printk("find_right_order 64:%d\n",find_right_order(64));
    printk("find_right_order 128:%d\n",find_right_order(128));

    printk("KMALLOC_CACHE_SIZE:%x\n",KMALLOC_CACHE_SIZE); // 0x80
    printk("KMEM_CACHE_MAX_WAST:%x\n",KMEM_CACHE_MAX_WAST); // 0x19a
    printk("KMALLOC_MINIMAL_SIZE_BIAS:%x\n",KMALLOC_MINIMAL_SIZE_BIAS); // 0x20
	for(i=0;i<KMALLOC_CACHE_SIZE;i++){ // 0x80
		if(kmem_cache_create(&kmalloc_cache[i],(i+1)*KMALLOC_MINIMAL_SIZE_BIAS,0)==NULL) // 32 , 64 ... , 4096
			return -1;
	}
	return 0;
}
...
void *kmalloc(unsigned int size){
	int index=kmalloc_cache_size_to_index(size); // 申请 32个字节 ,index = 1 // 对应 kmem_cache_create 时 的 第二个参数 (i+1)*KMALLOC_MINIMAL_SIZE_BIAS //  (i+1) >> 5
