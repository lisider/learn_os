...
/* --------------------------------------------------------------------------*/
/**
 * @Synopsis  根据 struct super_block 和 绝对路径返回 node
 *            在制作文件系统的时候，根据 绝对路径 和  struct super_block 生成了 node
 *
 * @Param block
 * @Param dir
 *
...
struct inode *simple_romfs_namei(struct super_block *block,char *dir){
	...
	dir=bmap(name,dir);
	if(block->device->dout(block->device,p,0,block->device->sector_size)!=block->device->sector_size)
	...
	while(1){
		...
		if(tmp>=block->device->storage_size)
			goto ERR_OUT_KMALLOC;
		if(tmp!=0){
			if(block->device->dout(block->device,p,tmp,block->device->sector_size)!=block->device->sector_size){
				goto ERR_OUT_KMALLOC;
			}
...
/* --------------------------------------------------------------------------*/
/**
 * @Synopsis  根据 node 可以获取当前文件的第一个字节地址.
 *
 * @Param node
 *
 * @Returns   
...
int romfs_init(void){
	...
	romfs_super_block.device=storage[RAMDISK]; // 不应该这么填,应该在register_file_system 时和 设备进行匹配
	return ret;
}
