...
struct task_info *current_task_info(void){
	register unsigned long sp asm ("sp"); // 307FEBFC
	return (struct task_info *)(sp&~(TASK_SIZE-1)); // 1111 1111 1111 1111    1111 0000 0000 0000 // 307FE000 // 初始化任务的 sp 的大小为 3068 字节 // 其他的都是3096
}
...
struct task_info *copy_task_info(struct task_info *tsk){
	...
	task_stack_base+=TASK_SIZE; // 4096 // 0x1000 // 0x30301000
	return tmp;
}
...
int do_fork(int (*f)(void *),void *args){ // 0 返回 307FE000
	struct task_info *tsk,*tmp;
	if((tsk=copy_task_info(current))==(void *)0) // 第一次返回 0x30300000 第二次返回0x30301000 第三次0x30301000
		return -1;

	tsk->sp=((unsigned int)(tsk)+TASK_SIZE); // 0x30300000 + 1000

    // 在 sp 中堆栈
    // 压栈 r15 r14 r12-r0 cpsr
